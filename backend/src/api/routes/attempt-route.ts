...
export async function attemptRoutes(app: FastifyInstance): Promise<void> {
  app.get<{ Params: { id: string } }>(
    '/attempts/:id',
    { schema: attemptStateResponseSchema, onRequest: [app.requireAttemptAccess] },
  ...
  app.put<{ Params: { id: string }; Body: SaveAnswerInput }>(
    '/attempts/:id/answers',
    { schema: saveAnswerSchema, onRequest: [app.requireAttemptAccess] },
    async (request) => {
  ...
  app.post<{ Params: { id: string } }>(
    '/attempts/:id/submit',
    { schema: submitAttemptSchema, onRequest: [app.requireAttemptAccess] },
    async (request) => container.attemptService.submit(request.user.sub, request.params.id),
  );

  app.get<{ Params: { id: string } }>(
    '/attempts/:id/result',
    { schema: attemptResultSchema, onRequest: [app.requireAttemptAccess] },
